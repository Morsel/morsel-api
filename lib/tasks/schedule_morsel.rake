...
namespace :schedual_morsel do
  ...
  task publish_morsel: :environment do
    ...
    CollageWorker.perform_async(morsel_id: morsel.id,place_id: 0,user_id: 3,post_to_facebook: false,post_to_twitter: false)
      # PublishMorselWorker.new.perform()
      # PublishMorsel.call(morsel: morsel,morsel_params: nil,social_params: {post_to_facebook: true,post_to_twitter: true},should_republish: false)
      # puts "done without error #{morsel}"
    end
  end
end
