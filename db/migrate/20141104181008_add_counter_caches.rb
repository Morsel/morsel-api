class AddCounterCaches < ActiveRecord::Migration
  ...
  def update_item_counter_caches
    ...
    Item.find_each do |i|
      i.update_columns comments_count: Comment.where(commentable_type:'Item', commentable_id:i.id).count
    end
  end
  ...
  def update_keyword_counter_caches
    ...
    Keyword.find_each do |k|
      k.update_columns followers_count: Follow.where(followable_id:k.id, followable_type:'Keyword').count
    end
  end
  ...
  def update_morsel_counter_caches
    ...
    Morsel.find_each do |m|
      m.update_columns likes_count: Like.where(likeable_type:'Morsel', likeable_id:m.id).count
    end
  end
  ...
  def update_place_counter_caches
    ...
    Place.find_each do |p|
      p.update_columns followers_count: Follow.where(followable_id:p.id, followable_type:'Place').count
    end
  end
  ...
  def update_user_counter_caches
    ...
    User.find_each do |u|
      u.update_columns  drafts_count: Morsel.drafts.where(creator_id: u.id).count,
