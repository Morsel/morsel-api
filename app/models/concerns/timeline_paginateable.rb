module TimelinePaginateable
  ...
  included do
    ...
    scope :after_date, -> (after_date, date_key, since_id = nil, klass = self) {
      where("(#{klass.table_name}.#{date_key}, #{klass.table_name}.id) > (to_timestamp(?), ?)", after_date, since_id) if after_date.present?
    }

    scope :max, -> (max_id, klass = self) { where(klass.arel_table[:id].lteq(max_id)) if max_id.present? }
    scope :before_date, -> (before_date, date_key, max_id = nil, klass = self) {
      where("(#{klass.table_name}.#{date_key}, #{klass.table_name}.id) < (to_timestamp(?), ?)", before_date, max_id) if before_date.present?
    }
    ...
    scope :paginate, -> (pagination_params, pagination_key = :id, klass = self) do
      if pagination_key == :id
        ...
        after_date(pagination_params[:after_date], pagination_key, pagination_params[:since_id], klass)
        .before_date(pagination_params[:before_date], pagination_key, pagination_params[:max_id], klass)
