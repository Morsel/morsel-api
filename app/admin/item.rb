ActiveAdmin.register Item do
  ...
  show do |item|
    attributes_table do
      ...
      row :description
      row 'Photo' do
        link_to(image_tag(item.photo_url(:_480x480)), item.photo_url, target: :_blank) if item.photo_url
      end
    end
  end
  ...
  form do |f|
    f.inputs 'Item' do
      f.input :description
      f.input :photo, hint: (f.template.image_tag(f.object.photo_url(:_480x480)) if f.object.photo_url)
    end
    f.actions
  end
end
